      ...
      <div className="text-center">
        ...
        <TransitionWrapper>
          <p className=" py-6 text-[#a0cdcd]">
            SwizzFunds has the perfect plan to help you achieve your financial
            goals.
          </p>
        </TransitionWrapper>
      </div>
      ...
      <div className="flex flex-wrap gap-10 mt-10">
        <div className="w-full bg-greenColor rounded-full p-2">
          <div className="flex justify-between items-center gap-1 xs:gap-8 text-xs lg:text-lg overflow-x-auto">
            {["Crypto", "Real Estate", "Forex", "Stock", "Gold", "Silver"].map((plan, index) => (
              <span
                key={index}
                onClick={() => handleToggle(index)}
                className={
                  activeIndex === index
                    ? "bg-white text-greenColor p-1 xs:p-4 lg:p-6 rounded-full w-full text-center font-semibold duration-700 whitespace-nowrap"
                    : "text-white p-4 lg:p-6 rounded-full w-full text-center font-semibold cursor-pointer whitespace-nowrap"
                }
              >
                {plan}
              </span>
